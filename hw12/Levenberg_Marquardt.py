...
def LevenbergMarquardt(Res_and_Jac,x,ITER_MAX,TOL):
    ...
    # start iterations
    iter = 1
    improvement=100#todo enter while
    while gradnorm > TOL and iter < ITER_MAX and improvement>0.1:
        ...
        # print(f"LM, iter #{iter}: loss = {lossvals[iter]}, gradnorm = {gradnorm:.4e}, rho = {rho:.4e}, R = {R:.4e} , improvement = {improvement:.4e}")
        iter = iter + 1
    print(f"LM, iter #{iter-1}: loss = {lossvals[iter-1]}, gradnorm = {gradnorm:.4e}, rho = {rho:.4e}, R = {R:.4e} , improvement = {improvement:.4e}")
